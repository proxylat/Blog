...
const htmlFiles = async d => {
  ...
  const files = await Promise.all(entries.map(e => {
    ...
    return e.isDirectory()
      ? htmlFiles(res)
      : (e.isFile() && path.extname(e.name).toLowerCase() === '.html' && !res.includes('report.html') ? res : []);
  }));
  return files.flat();
};
...
const run = async () => {
  ...
  const html = htmlParts.map(h => '<div>' + h + '</div>').join(' ');
  ...
  try {
    const result = dropcss({
      ...
      shouldDrop: (sel) => {
        ...
        if (sel.includes('.js-plotly-plot')) return false;
        return true;
      },
    });
    purged = result.css;
